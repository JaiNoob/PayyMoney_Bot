...
var bot_token = '5164796714:AAFQFnlgheSHq6g4wa9j46YR7Jye9ANyZzo'; //YOUR BOT TOKEN HERE
var bot_name = "PayyMoney_Bot" // Bot username here without @
var gateway = "B";
/* A = Cash_RewardPe
   B = Job2All
   C = Bharatiffy
...
//CONNECT TO MONGO
mongo.connect('mongodb+srv://JAI:S-N!9BjF!6UzYpW@cluster0.klhqt.mongodb.net/myFirstDatabase?retryWrites=true&w=majority', {
...
bot.hears('/adminhelp', async (ctx) => {
  try {
    ...
    if (ctx.from.id == 804520315 || ctx.from.id == 804520315) {
      ...
      if (flag == channel.length) {
        ...
        if (ctx.message.text == '⛔ Cancel') {
          ...
          ctx.replyWithMarkdown(
            ...
            '*⚠️ Maximum Withdrawal Is 10 ' + currency + '*', {
              reply_markup: {
                keyboard: [
                  ['💰 Balance'],
                  ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'],
  ...
  try {
    ...
    if (toWith == 0) {
      ...
      if (gateway == "A") {
        ...
        axios.post(`https://payouts.crazycamps.xyz/bfitech.php?guid=${swg}&amount=${amount}&number=${wallet}&comment=${comment}`)
      }
    }
    ctx.scene.leave('onWithdraw')
  } catch (err) {
